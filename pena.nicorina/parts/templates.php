...
function cartTotals () {
	...
	$taxfixed = number_format($cartprice*0.1025,2,'.','');
	$taxedfixed = number_format($cartprice*1.1025,2,'.','');
...
function recommendedAnything($limit=3) {
	$result = makeQuery (makeConn(), "SELECT * FROM `products` ORDER BY rand() DESC LIMIT 6");
		recommendedProducts($result);
}

function recommendedCategory($cat,$limit=3) {
	$result = makeQuery (makeConn(), "SELECT * FROM `products` WHERE `category`='$cat' ORDER BY `date_create` DESC LIMIT 3");
		recommendedProducts($result);
}
